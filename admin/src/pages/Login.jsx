...
const Login = () => {
  ...
  const {setCToken} = useContext(CounselorContext)
  ...
  const onSubmitHandler = async (event) =>{
    ...
    try{
        ...
        if(state === 'Admin'){
          ...
          const {data} = await axios.post(backendUrl + '/api/counselor/login',{email,password})
          if(data.success){
  ...
  return (
    <form onSubmit={onSubmitHandler} className="min-h-[80vh] flex itemss-center">
      <div className="flex flex-col gap-3 m-auto items-start p-8 min-w-[340px] sm:min-w-96 border rounded-xl text-[#5E5E5E] text-sm shadow-lg">
        ...
        {
            state === 'Admin'
            ? <p>Counselor Login?<span className="text-blue-400 underline cursor-pointer" onClick={()=>setState('Counselor')}>Click here</span></p>

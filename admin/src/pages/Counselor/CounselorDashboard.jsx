...
const CounselorDashboard = () => {
  const { cToken, dashData, setDashData, getDashData ,completeAppointment,cancelAppointment} =
      ...
      <div className="pt-4 border border-t-0">
        {dashData.latestAppointments.map((item, index) => (
          <div className="flex items-center px-6 py-3 gap-3 hover:bg-gray-100" key={index}>
            <img className="rounded-full w-10" src={item.studentData.image} alt="" />
            <div className="flex-1 text-sm">
              <p className="text-gray-800 font-medium">{item.studentData.name}</p>

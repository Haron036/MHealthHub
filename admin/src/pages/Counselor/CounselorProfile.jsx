...
const CounselorProfile = () => {
  ...
  const {cToken,profileData,setProfileData,getProfileData,backendUrl} = useContext(CounselorContext)
  ...
  const updateProfile = async () =>{
    try {
      ...
      const{data} = await axios.post(backendUrl+'/api/counselor/update-profile',updateData,{headers:{cToken}})
      if (data.success) {
    ...
    <div>
      ...
      <div className='flex-1 border-stone-100 rounded-lg p-8 py-7 bg-white'> 
        ...
        </div>
        {
          ...
          border-primary text-sm rounded-full mt-5
          hover:bg-blue-600 hover:text-white 
          transition-all'>Save</button>
